...
export default function S4TheoryC3Responsibility() {
  const rows = [
    { stage: 'allow', resp: '에이전트', meaning: '자율에 맡김' },
    { stage: 'ask', resp: '함께', meaning: '작업자 + 에이전트 결정' },
  ...
  return (
    <>
      ...
      <div className="rounded border border-divider overflow-hidden max-w-5xl">
        <div className="grid grid-cols-[1.2fr_1.4fr_2fr] bg-bg-soft border-b border-divider">
          <div className="px-5 py-3 text-sm uppercase tracking-wider text-text-sub">단계</div>
          <div className="px-5 py-3 text-sm uppercase tracking-wider text-text-sub border-l border-divider">책임</div>
          <div className="px-5 py-3 text-sm uppercase tracking-wider text-text-sub border-l border-divider">의미</div>
        </div>
        {rows.map((r, i) => (
          <div
            key={i}
            className={`grid grid-cols-[1.2fr_1.4fr_2fr] ${i < rows.length - 1 ? 'border-b border-divider' : ''}`}
          >
            <div className="px-5 py-4 text-base text-text font-bold">{r.stage}</div>
            <div className="px-5 py-4 text-base text-text border-l border-divider">{r.resp}</div>
